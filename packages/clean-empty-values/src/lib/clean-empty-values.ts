...
class Cleaner {
  ...
  clean<T extends Record<string, unknown> | unknown | unknown[]>(data: T): Partial<T> | T {
    if(!data) {
      return data;
    }
    ...
    if(this._isObject(data)) {
      return (this.options.cleanInPlace? this._cleanObjectInPlace(data) : this._cleanObject(data)) as Partial<T>;
    }
  ...
  private _cleanObjectInPlace<T extends Record<string, unknown>>(object: T): Partial<T> {
    Object.keys(object).forEach((key) => {
      const value = object[key];

      if (Array.isArray(value)) {
        const mappedArr = this._cleanArray(value);
        if(this._shouldReturnArray(mappedArr)) (object as Record<string, unknown>)[key] = mappedArr;
  ...
  private _cleanObject<T extends Record<string, unknown>>(object: T): Record<string, unknown> {
...
export function cleanEmptyValues<Explicit = "no_explicit_defined", Inferred = unknown>(object: Inferred, options: CleanerOptions) {
  return new Cleaner(options).clean(object) as Explicit extends "no_explicit_defined" ? Partial<Inferred> : Explicit;
}
