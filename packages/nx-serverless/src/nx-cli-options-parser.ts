...
export function resolvedOptionsFn<
  T extends BaseServerlessExecutorSchema,
  C extends ExecutorContext
>(options: T, context: C) {
...
export function printInputOptions<T extends BaseServerlessExecutorSchema>(
  serverless: Serverless,
  options: T
): void {
  const relativeConfigurationPath = path.join(
    options.cwd,
    options['serverless-file-ext']
  );
  ...
  const message =
    '\nNX resolved options:\n' +
    '------------------------\n' +
    `${prettyjson.render(serverlessStats, {
      noColor: options['no-colors'],
